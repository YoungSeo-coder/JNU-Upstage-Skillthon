...
<<<<<<< HEAD
description: 근로계약서 이미지/PDF를 분석하여 핵심 항목을 추출하고 법적 준수 여부를 검증하는 AI Skill. 사용자가 '근로계약서 분석 탭'에 사진이나 파일을 업로드하면, AI가 자동으로 시급·휴게시간·주휴수당 등 핵심 내용을 파악하여 최저시급 미준수·독소조항 등의 위반 사항을 즉시 탐지하고 시각적인 분석 결과를 제공한다.
=======
...
<<<<<<< HEAD
Upstage Document Parse API로 근로계약서를 텍스트화하고, Solar LLM으로 핵심 항목을 추출한 뒤 5개 준수 항목 기준 대비 준수 여부를 반환한다.
=======
...
| compliance_check 키 | 검증 방법 |
|---|---|
| `최저시급_준수` | 추출된 시급 vs 2026년 최저시급 (10,030원) |
| `휴게시간_준수` | 근기법 제54조 기준 (4h→30분, 8h→1시간) |
| `주휴수당_명시` | 계약서 내 주휴수당 언급 여부 |
| `수습기간_합법성` | 수습 급여 삭감 조항 + 계약기간 1년 미만이면 위반 |
| `독소조항_없음` | 근로기준법 제20조 위반 위약금·손해배상 강제조항 존재 여부 |

## 리스크 등급

| 등급 | 기준 |
|---|---|
| HIGH | 최저시급 미준수, 수습기간 최저임금법 위반(1년 미만 계약), 위약금 및 손해배상 강제조항 중 하나라도 확인 |
