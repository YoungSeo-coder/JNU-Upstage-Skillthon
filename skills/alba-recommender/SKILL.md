...
프론트엔드에서 사용자가 직접 체크한 O/X 결과를 받아 클린지수를 산출한다. LLM 분석 없이 체크리스트 데이터만으로 단순 산수로 계산한다.

```bash
# stdin JSON — 프론트엔드 연동 주요 방식 (criteria_flags: O/X 체크 결과)
echo '{
...
python ./scripts/add_review.py 스타벅스_상대점 --violations "휴게시간 부족" --review "쉬는 시간이 없었어요."

# 신규 사업장 등록
python ./scripts/add_review.py --name "카페 온도" --area 후문 --industry 카페 --violations "급여 지급 지연" --review "리뷰 텍스트"
```
...
| 7 | 반복적이고 지속적인 대타 요구 및 강요 | 합리적인 대타 요청 |
